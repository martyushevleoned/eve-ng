...
sudo 'include custom.rules' >> /etc/snort/rules/snort.conf

sudo vim /etc/snort/custom.rules
...
```
```shell
wget https://raw.githubusercontent.com/eldondev/Snort/refs/heads/master/schemas/create_mysql
sudo mysql -u root -p snort < create_mysql
```
* настройка TAP-Sensor
```shell
...
sudo make install
```
* Настройка BASE на Control Panel
```shell
sudo apt install -y apache2 php libapache2-mod-php php-mysql php-gd php-pear php-cli libphp-adodb
...
```
```
open http://localhost/base/setup/index.php

ADODB path: /usr/share/php/adodb
Database Name: snort
Database Host: localhost
Database Port: (оставьте пустым или 3306)
Database User: snort
